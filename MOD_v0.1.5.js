(function () {
    ...
    var FtHoFPlanner = {
        ...
        /*
         * コンボ検索
         * 4つの効果が10手の中にあれば、該当する手を赤紫でハイライトする。
         * 順番は問わず、シーズンを跨ぐ設定も可能。
         */
        getComboMatchedCells: function (forecasts) {
            var settings = this.comboSettings;
            var allSelected = true;

            for (var s = 0; s < settings.length; s++) {
                if (!settings[s].effect) {
                    allSelected = false;
                    break;
                }
            }
            ...
            for (var i = 0; i < forecasts.length; i++) {
                ...
                var pairs = [
                    { season: 'season1', forecast: item.forecast },
                    { season: 'season2', forecast: item.season2Forecast }
                ];

                for (var j = 0; j < pairs.length; j++) {
                    var pair = pairs[j];
                    var forecast = pair.forecast;
            ...
            function searchPart(partIndex, used) {
                if (partIndex >= settings.length) {
                    return true;
                }

                var setting = settings[partIndex];

                for (var oi = 0; oi < occurrences.length; oi++) {
                    var occurrence = occurrences[oi];

                    if (used[oi]) continue;
                    if (occurrence.effect !== setting.effect) continue;
                    if (!this.comboGcMatches(occurrence.gc, setting)) continue;

                    if (!this.comboCrossSeason && selected.length > 0 &&
                        occurrence.season !== selected[0].season) {
                        continue;
                    }
                    ...
                    if (searchPart.call(this, partIndex + 1, used)) {
                        return true;
                    }
            ...
            if (searchPart.call(this, 0, {})) {
                for (var m = 0; m < selected.length; m++) {
                    matched[selected[m].season][selected[m].index] = true;
                }
            }
        ...
        updateOptionsMenu: function () {
            ...
            for (
                 ...
                 * コンボ検索はハイライト設定より優先。
                 */
                if (comboMatchedCells.season1[i]) {
                    season1HighlightStyle =
                        'background:#d98bb3;';
                }

                if (comboMatchedCells.season2[i]) {
                ...
                '<div class="listing" style="margin-top:4px;">' +
                    ...
                    ';margin-top:8px;">' +
                        ...
                        '<div style="font-size:12px;opacity:0.85;margin-bottom:6px;">' +
                            '4つの効果が10手以内に、順番を問わず入っていれば赤紫でハイライトします。' +
                        '</div>' +
                        (function () {
                            var html = '';
                            var effects = [
