(function () {
    ...
    var FtHoFPlanner = {
        ...
        // 予測結果キャッシュ。5秒ごとのUpdateMenuで同じ予測を再計算しない。
        forecastCache: {},
        forecastCacheKey: '',
        ...
        /*
         * マイセッティングのローカルバックアップ
         * MODローダーによる再読み込みなどでGame.registerModのloadが
         * 呼ばれない場合でも、保存済み設定を復元できるようにする。
         */
        mySettingsBackupKey: 'FtHoFPlanner_MySettings_Backup',

        saveMySettingsBackup: function () {
            try {
                localStorage.setItem(
                    this.mySettingsBackupKey,
                    JSON.stringify({
                        version: 1,
                ...
                console.warn('[FtHoF Planner] マイセッティングのバックアップ保存に失敗しました。', error);
            }
        },

        loadMySettingsBackup: function () {
            try {
                var backup = localStorage.getItem(this.mySettingsBackupKey);
                ...
                console.warn('[FtHoF Planner] マイセッティングのバックアップ読み込みに失敗しました。', error);
                return false;
            }
        },
        ...
        forecastMany: function (count) {
            ...
            /*
             * 予測結果に影響する値だけをキーにする。
             * 5秒更新でここが変わっていなければ、500手分のRNG計算をやり直さない。
             */
            var cacheKey = JSON.stringify({
                seed: Game.seed || '',
                startCount: startCount,
                count: count,
                goldenCount: this.getGoldenCount(),
                buildingsOwned: Game.BuildingsOwned || 0,
                settings: this.settings
            });

            if (this.forecastCacheKey === cacheKey && this.forecastCache[count]) {
                return this.forecastCache[count];
            }
            ...
            this.forecastCache[count] = forecasts;
        ...
        updateOptionsMenu: function (force) {
                ...
                (function (index) {
                    ...
                    if (autoCheckbox) {
                        autoCheckbox.onchange = function () {
                            ...
                            FtHoFPlanner.saveMySettingsBackup();
            ...
            for (var settingIndex = 0; settingIndex < settingIds.length; settingIndex++) {
                (function (id) {
                    ...
                    checkbox.onchange = function () {
                        ...
                        FtHoFPlanner.forecastCache = {};
                        FtHoFPlanner.forecastCacheKey = '';
                        FtHoFPlanner.updateOptionsMenu(true);
                    };
            ...
            // 500手分の予測配列を毎回consoleへ出すと重くなるため、出力しない。
        }
    };
     ...
     */
    /*
     * MODローダーから再読み込みされた場合など、Game.registerModのloadが
     * まだ呼ばれていないケースではローカルバックアップから復元する。
     * 既に設定が復元済みなら上書きしない。
     */
    if (
        FtHoFPlanner.mySettings[0] === null &&
        FtHoFPlanner.mySettings[1] === null &&
        FtHoFPlanner.mySettings[2] === null
    ) {
        FtHoFPlanner.loadMySettingsBackup();
    }
